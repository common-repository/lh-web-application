...
const imagesCacheName = '<?php echo self::return_plugin_namespace(); ?>-images';
const maxPages = 100; // Maximum number of pages to cache
const maxImages = 1000; // Maximum number of images to cache
const timeout = 7000; // Number of milliseconds before timing out
...
function updateStaticCache() {
    ...
    .then( pagesCache => {
        ...
        return pagesCache.addAll([
            '<?php echo get_permalink($this->options[ $this->offline_page_field_name ]); ?>'
        ]);
    });
}
        ...
        event.respondWith(
            new Promise( resolveWithResponse => {
                ...
                .catch( fetchError => {
                    ...
                    .then( responseFromCache => {
                        resolveWithResponse(
                            responseFromCache || caches.match('<?php echo get_permalink($this->options[ $this->offline_page_field_name ]); ?>')
                        );
                    });
                });
    ...
    event.respondWith(
        ...
        .then(responseFromCache => {
            ...
            .then( responseFromFetch => {
                ...
                if (request.url.match(/\.(jpe?g|png|gif|svg|mapbox|ico|css)/)) {
                    const copy = responseFromFetch.clone();
                    try {
                        event.waitUntil(
            ...
            .catch( fetchError => {
              console.log(request.url + 'failed to fetch');
                //console.error(fetchError);
                // FALLBACK
                // show an offline placeholder
                if (request.url.match(/\.(jpe?g|png|gif|svg|mapbox|ico)/)) {
